...
def step(pos, seen, facing, cost, path, grid):
    ...
    costmod = 1001
    #This was to over and edge case, but there is clearly at least one more that happens in my real input but not the tests
    #I dont know what It is and im not going to spend more time looking
    #my guess, is some + intersection
    if grid[pos] != "E" and grid[addTuple(pos, facing)] == "#" and (grid[addTuple(pos, turns[facing][0])] != "#" or grid[addTuple(pos, turns[facing][1])] != "#"):
        cost += 1000
        costmod = 1
    ...
    if grid[addTuple(pos, turns[facing][0])] != "#" and seen[addTuple(pos, turns[facing][0])] >= (cost + costmod):
        options.append(step(addTuple(pos, turns[facing][0]), seen, turns[facing][0], cost+costmod, path, grid))
        
    if grid[addTuple(pos, turns[facing][1])] != "#" and seen[addTuple(pos, turns[facing][1])] >= (cost + costmod):
        options.append(step(addTuple(pos, turns[facing][1]), seen, turns[facing][1], cost+costmod, path, grid))
